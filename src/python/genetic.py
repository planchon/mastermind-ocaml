...
def genetic_evolution(popsize, generations, costfitness,
    ...
    generations: maximum number of population generations
    ...
    while len(chosen_ones) <= popsize and h <= generations:
        ...
        pop_score = []
        for son in sons:
            pop_score.append((costfitness(son), son))
        ...
        for code in eligibles:
            if code in chosen_ones:
                ...
                chosen_ones.append(
                    [random.randint(1, len(COLORS)) for i in range(slots)])
...
# prend le coup a jouer, le nombre de coups, le code secret retourne le code sous forme de tuple
...
def main():
    ...
    while result != (4, 0):  # while X_i \neq P
        ...
        eligibles = genetic_evolution(
            MAX_POP_SIZE, MAX_GENERATIONS, scoref, slots=4)
        ...
        # prend le plus dernier des code qui n'a jamais été testé
        code = eligibles.pop()
